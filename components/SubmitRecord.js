...
const PhotoUploadUI = ({ previews, onPhotoChange, disabled }) => (
    React.createElement('div', { className: "flex flex-col p-4 border-2 border-slate-600 border-dashed rounded-md h-full text-center" },
        ...
        React.createElement('div', null,
            React.createElement('label', { htmlFor: 'record-photo-upload', className: `relative cursor-pointer bg-slate-700 rounded-md font-medium text-orange-400 hover:text-orange-300 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-offset-slate-800 focus-within:ring-orange-500 px-2 ${disabled ? 'pointer-events-none opacity-50' : ''}` },
                React.createElement('span', null, "사진 업로드"),
                React.createElement('input', { id: 'record-photo-upload', name: 'record-photo-upload', type: "file", multiple: true, className: "sr-only", onChange: onPhotoChange, accept: "image/*", disabled: disabled })
            ),
            React.createElement('p', { className: "text-xs text-slate-500 mt-1" }, "5MB 이하")
        )
    )
);
...
const SubmitRecord = ({ onSubmitSuccess, onRegisterClick }) => {
  ...
  const handlePhotoChange = useCallback(async (e) => {
    if (e.target.files) {
      ...
      const dataUrls = await Promise.all(files.map(f => URL.createObjectURL(f)));
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    const success = await addRecord(selectedUserId, distanceNum, recordPhotos);
    setIsSubmitting(false);
    if (success) {
      onSubmitSuccess();
    }
  }, [selectedUserId, distance, recordPhotos, addRecord, onSubmitSuccess]);

  if (isLoading && users.length === 0) {
    return React.createElement('div', { className: "text-center p-10" }, "사용자 목록을 불러오는 중...");
  }
  ...
  return (
    React.createElement('div', { className: "bg-slate-800/80 p-6 sm:p-8 rounded-xl shadow-2xl border border-slate-700 backdrop-blur-sm" },
      ...
      React.createElement('form', { onSubmit: handleSubmit, className: "space-y-6" },
        React.createElement('div', null,
          React.createElement('label', { className: "block text-sm font-medium text-slate-300 mb-2" }, "이름"),
              ...
              isDropdownOpen && (
                React.createElement('ul', { className: "absolute z-20 mt-1 w-full bg-slate-800 border border-slate-600 rounded-md shadow-lg max-h-60 overflow-auto" },
                  sortedUsers.map(user => (
                    React.createElement('li', { key: user.id, onClick: () => { setSelectedUserId(user.id); setIsDropdownOpen(false); }, className: "flex items-center px-3 py-2 text-sm text-white hover:bg-slate-700 cursor-pointer" },
        ...
        React.createElement('div', { className: `space-y-6 transition-opacity duration-300 ${!selectedUserId ? 'opacity-50 pointer-events-none' : 'opacity-100'}` },
          React.createElement('div', null,
            React.createElement('label', { htmlFor: "distance", className: "block text-sm font-medium text-slate-300 mb-2" }, "달린 거리 (km)"),
            React.createElement('input', { id: "distance", type: "number", step: "0.01", value: distance, onChange: (e) => setDistance(e.target.value), className: "w-full bg-slate-700 border border-slate-600 rounded-md py-2 px-3 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500", placeholder: "10.5", disabled: !selectedUserId })
          ),
          React.createElement('div', null,
